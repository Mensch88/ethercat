				      ...
				      struct net_device *dev)
{
	...
	if (unlikely(stop_queue)) {
		 ...
		 */
		smp_mb();
		if (!tp->ecdev && rtl_tx_slots_avail(tp, MAX_SKB_FRAGS))
...
//				tp->Rx_databuff[entry] = page_address(tp->Rx_databuff[entry]);
//				rx_buf = tp->Rx_databuff[entry];
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance)
{
	...
	if (!tp->ecdev && unlikely(status & SYSErr)) {
